<?php
function ii_rebuild_txtimg($fonttext,$id=''){
  $txtbg = ii_random(22).'.png';
  $img = ii_get_actual_route('common/images/public/icon/').rand(1,51).'.png';
  global $ngenre,$nuppath;
  $outimg = ii_get_actual_route($ngenre).'/'.$nuppath.'txtimg/';
  if (!(is_dir($outimg))) @mkdir($outimg, 0775);
  if(!ii_isnull($id)) $outimg .= ii_md5(ii_md5($ngenre.'_'.$id).ii_md5($fonttext,1)).'.jpg';
  else $outimg .= ii_md5($ngenre.ii_md5($fonttext,1)).'.jpg';
  if(file_exists($outimg)) return $outimg;
  $fontface = ii_get_actual_route('common/images/font/').'NotoSansHans-Regular.otf';
  $rot = 0; // 旋转角度
  $width = 0; //宽度
  $height = 0; //高度
  $offset_x = 0; //x偏移
  ...
  $fontsize = 18;//文字大小写
  $bounds = array();
  $fonttexts = ii_autowidth_text($fontsize, $rot, $fontface,$fonttext, $fontwidth); // 自动换行处理
  /** [字体大小] [角度] [字体名称] [字符串] [预设宽度] */
  // 确定边框高度.
  $bounds = ImageTTFBBox($fontwidth, $rot, $fontsize, "W");
  if ($rot < 0) {
  ...
  // 确定边框高度.
  $bounds = ImageTTFBBox($fontwidth, $rot, $fontsize, $fonttexts);
  if ($rot < 0) {
  ...
  if(ii_rebuild_img($img,$txtbg)){
    $txtbg_c = imagecreatefromstring(file_get_contents($txtbg));
    @unlink($txtbg);
    $bg_rgb = ii_get_imgrgb($img);
    $red = $bg_rgb['red'];
    $green = $bg_rgb['green'];
    $blue = $bg_rgb['blue'];
    $fontcolor = imagecolorallocate($txtbg_c, $red, $green, $blue); // 创建文字颜色
    ImageTTFText($txtbg_c, $fontsize, $rot, 30, 125, $fontcolor, $fontface, $fonttexts);
    imagepng($txtbg_c,$outimg);
    imagedestroy($txtbg_c);
    return $outimg;
  }
}

function ii_rebuild_img($img,$out){
  $out_bottom = ii_random(8).'.png';
  $out_border = ii_random(8).'.png';
  $bool = false;
  $image = imagecreatefromstring(file_get_contents($img));
  $imgWidth = imagesx($image);
  $imgHeight = imagesy($image);
  $bottom = imagecreatetruecolor($imgWidth,$imgHeight); // 创建画布
  $bottom_color=imagecolorallocatealpha($bottom,255,255,255,36);//($image,$red,$green,$blue,$alpha),$alpha 0表示完全透明，127表示完全不透明。
  imagefill($bottom , 0 , 0, $bottom_color);//填充画布
  imagesavealpha($bottom ,true);
	if(imagecopy($bottom,$image, 0, 0, 0, 0, $imgWidth, $imgHeight)){
    imagepng($bottom, $out_bottom);
    imagedestroy($bottom);
    imagedestroy($image);
    $bool = true;
  }
  $img_rgb = ii_get_imgrgb($img);
  $border_rgb['red'] = 255-$img_rgb['red'];
  $border_rgb['green'] = 255-$img_rgb['green'];
  $border_rgb['blue'] = 255-$img_rgb['blue'];
  if($bool){
    //添加边框代码段
    $border = imagecreatetruecolor($imgWidth + 10,$imgHeight + 10); // 创建画布
    $bordercolor=imagecolorallocatealpha($border ,$border_rgb['red'], $border_rgb['green'], $border_rgb['blue'],0);//($image,$red,$green,$blue,$alpha),$alpha 0表示完全透明，127表示完全不透明。
    imagefill($border , 0 , 0, $bordercolor);//填充画布
    imagesavealpha($border ,true);
    $out_bottom_c = imagecreatefromstring(file_get_contents($out_bottom));
    @unlink($out_bottom);
    if(imagecopy($border,$out_bottom_c, 5, 5, 0, 0, $imgWidth, $imgHeight)){
      imagepng($border,$out_border);
      imagedestroy($border);
      imagedestroy($out_bottom_c);
      $bool = true;
    }
    else $bool = false;
    //添加边框代码段
  ...
  if($bool){
    //添加大底图代码段
    $bg = imagecreatetruecolor(600,371); // 创建画布
    $bgcolor=imagecolorallocatealpha($bg,225,225,225,0);//($image,$red,$green,$blue,$alpha),$alpha 0表示完全透明，127表示完全不透明。
    imagealphablending($bg ,false);//关闭混合模式，以便透明颜色能覆盖原画布
    imagecolortransparent($bg,$bgcolor);
    imagefill($bg , 0 , 0, $bgcolor);//填充
    imagesavealpha($bg ,true);//设置保存PNG时保留透明通道信息
    $out_border_c = imagecreatefromstring(file_get_contents($out_border));
    @unlink($out_border);
    if(imagecopymerge($bg,$out_border_c, 335, 75, 0, 0, $imgWidth + 10, $imgHeight + 10,100)){
      imagepng($bg, $out);
      imagedestroy($bg);
      imagedestroy($out_border_c);
      $bool = true;
    }
    else $bool = false;
    //添加大底图代码段
...
function ii_autowidth_text($fontsize, $rot, $fontface, $string, $width) {
  $content = "";
  ...
  foreach ($letter as $l) {
    $teststr = $content." ".$l;
    $testbox = imagettfbbox($fontsize, $rot, $fontface, $teststr);
    // 判断拼接后的字符串是否超过预设的宽度
    if (($testbox[2] > $width) && ($content !== "")) $content .= PHP_EOL;
    $content .= $l;
  }
  return $content;
}

function ii_get_imgrgb($image)
{
  $bg_image = imagecreatefromstring(file_get_contents($image));
  $i = imagecreatefromjpeg($image);
  $rTotal = $gTotal = $bTotal = $total = 0;
  $i      = imagecreatefromjpeg($image);
  for ($x = 0; $x < imagesx($bg_image); $x++) {
      for ($y = 0; $y < imagesy($bg_image); $y++) {
          $rgb    = imagecolorat($bg_image, $x, $y);
          $r      = ($rgb >> 16) & 0xFF;
          $g      = ($rgb >> 8) & 0xFF;
          $b      = $rgb & 0xFF;
          $rTotal += $r;
  ...
  return ['red'=> $rAverage,'green'=> $gAverage,'blue'=> $bAverage];
}
...
function ii_get_Infos($infos,$tpl='')
{
  ...
  if (is_array($arys))
  {
    ...
    foreach ($arys as $info)
    {
      ...
      $tmptstr = str_replace('{$order}', $info[1], $tmptstr);
      $tmptstr = str_replace('{$title}', $info[2], $tmptstr);
      $tmptstr = str_replace('{$content}', $info[3], $tmptstr);
...
function ii_get_Features($features,$tpl='')
{
  $arys = json_decode($features,true);
  if (is_array($arys))
  {
    if(!ii_isnull($tpl)) $tmpstr = ii_itake('global.tpl_transfer.'.$tpl, 'tpl');
    else $tmpstr = ii_itake('global.tpl_transfer.features', 'tpl');
    $tmpastr = ii_ctemplate($tmpstr, '{@}');
    $i = 0;
    foreach ($arys as $feature)
    {
      $tmptstr = $tmpastr;
      $left = 'left';
      $center = 'center';
      ...
      if($i==0) $class = $center;
      $tmptstr = str_replace('{$id}', $feature[0], $tmptstr);
      $tmptstr = str_replace('{$order}', $feature[1], $tmptstr);
      $tmptstr = str_replace('{$class}', $class, $tmptstr);
      $tmptstr = str_replace('{$title}', $feature[2], $tmptstr);
      $tmptstr = str_replace('{$image}', $feature[3], $tmptstr);
      $tmptstr = str_replace('{$content}', str_replace("\n","</br>",$feature[4]), $tmptstr);
      $tmptstr = ii_creplace($tmptstr);
      $tmprstr .= $tmptstr;
      $i++;
    }
...
function ii_require_ApiFile($path){
 ...
 foreach($arys as $ary){
  if(is_file($ary)) require_once($ary);
 }
}
...
function ii_cache_html($name)
{
  global $nurltype;
  if($nurltype == 0) $path = 'php';
  else $path = 'html';
  $cache_dir = ii_get_actual_route('./') . CACHE_DIR;
  $cache_filename = $cache_dir . '/'.$path.'/' . $name . '.inc.php';
  if (file_exists($cache_filename))
  {
    $mtime = filemtime($cache_filename);//filectime创建时间，filemtime更新时间
    if(ii_check_expireDate(date('Y-m-d H:i:s',$mtime),'7','3')) return false;//7天更新一次缓存，最后一个数字：3天，5分钟,6秒
    else return true;
  }
  else
  {
...
function ii_cache_get($name, $type)
{
  ...
  if($type == 2) $filename = ii_get_actual_route('./') . CACHE_DIR . '/'.$path.'/' . $name . '.inc.php';
  else $filename = ii_get_actual_route('./') . CACHE_DIR . '/' . $name . '.inc.php';
...
{
  global $nurltype, $nphpcache;
  if($type == 2 && $nurltype == 0 && $nphpcache == 0) return;//停用动态缓存
  if($nurltype == 0) $path = 'php';
  else $path = 'html';
  $cache_dir = ii_get_actual_route('./') . CACHE_DIR;
  if (!(is_dir($cache_dir))) @mkdir($cache_dir, 0700);
  if (!(is_dir($cache_dir . '/'.$path))) @mkdir($cache_dir . '/'.$path, 0700);
  $cache = new cc_cache;
  $cache -> cachename = $name;
  if($type == 2) $filename = $cache_dir . '/'.$path.'/' . $name . '.inc.php';
  else $filename = $cache_dir . '/' . $name . '.inc.php';
  $cache -> filename = $filename;
  switch ($type)
  {
    case 0:
...
function ii_cache_remove($name = '' , $dir = '')
{
  $cache_dir = ii_get_actual_route('./') . CACHE_DIR;
  if (!ii_isnull($dir))
  {
...
function ii_get_active_things($type)
{
  ...
  if (!(ii_isnull($tthings)))
  {
    ...
    if (!ii_isnull($trthings) && ii_isRobotsAgent() && $tthings == 'skin') $trthings = $GLOBALS['robots_skin'];
...
function ii_iurl($type, $urlid, $set, $strers = '')
{
  ...
  $ucode = is_numeric($urlid) ? mm_get_field($ngenre,$urlid,'ucode') : '';
  $tset = ii_get_num($set);
...
function ii_md5($strers,$type = 0)
{
  if($type == 0){
    return md5($strers);
  }else{
      $charset = mb_detect_encoding($strers,"UTF-8,GB2312,GBK");
      $strers = iconv($charset,"UTF-8",$strers);
      return md5($strers);
  }
}
...
function ii_replace_xinfo_ary($strers, $type)
{
  ...
  switch($type)
  {
    case 'tpl':
      ...
      if (!is_dir(ii_get_actual_route($trootdir))) $troot = 'common/template/notes';//如果后台配置的模板不存在,则使用notes模板.
