...
function mm_update_count($genre,$id)
{
  //更新count字段
  ...
  $tfield = 'count';
  $tcount = mm_get_field($tgenre,$id,$tfield);
  $val = $tcount + 1;
  $tsqlstr = 'update '. $tdatabase.' set '.ii_cfnames($tfpre,$tfield).' = '.$val.' where '.$tidfield.' = ' .$id;
...
function mm_get_sortfiled($genre, $lng, $id, $filed)
{
  $tary = mm_get_sortary($genre, $lng);
  if (is_array($tary))
  {
...
function mm_web_title($title)
{
  ...
  if (!ii_isnull($ttitle)) {
    ...
    else $tweb_title = $ttitle . SP_STR . $tweb_title;
  }else{
    if(!ii_isnull($ngenre)) $tplus = SP_STR . ii_itake('global.'.$ngenre.':module.channel_title', 'lng');
    $tweb_title = $tplus . $tweb_title;
  }
  return $tweb_title;
}
...
function vv_itransfer($type, $tpl, $vars)
{
  ...
  $tweek = ii_get_num(ii_get_strvalue($vars, 'week'));
  $tday = ii_get_num(ii_get_strvalue($vars, 'day'));
      ...
      if (ii_isnull($tbsql))
      {
        switch($type)
        {
          case 'rand':
            ...
            break;
          case 'timetable':
            $tsqlstr = "select * from $tdatabase where " . ii_cfnames($tfpre, 'hidden') . "=0 and " . ii_cfnames($tfpre, 'week') . "=".$tweek." and " . ii_cfnames($tfpre, 'day') . "=".$tday;
            $tsqlorder = " order by " . ii_cfnames($tfpre, 'start_time') . " asc";
            break;
          case 'all':
            $tsqlstr = "select * from $tdatabase where 1=1";
      ...
      if(!ii_isnull($tdatabase)){
        ...
        if (!ii_isnull($tmpstr))
        {
          ...
          if ((empty($tmprstr) && $type == 'up')||(empty($tmprstr) && $type == 'down')) {
            $tipsnull = '<a><span class="meta-nav"><span class="post-nav">'.$tips.'</span><br/>'.ii_itake('global.lng_config.nomore','lng').'</span></a>';
...
{
  global $variable,$nurltype,$nlng,$ntitle,$nurl,$nurs,$nurlpre;
...
function wdja_cms_init($route)
{
  ...
  global $nvalidate, $nurltype, $nphpcache, $ncreatefiletype;
  $nvalidate = $variable['common.nvalidate'];
  $nurltype = $variable['common.nurltype'];
  $nphpcache = ii_get_num($variable['common.nphpcache'],0);
