...
function wdja_cms_admin_manage_adddisp()
{
  ...
  $isallday = ii_get_num($_POST['isallday']);//是否是全天事件
  $isend = ii_get_num($_POST['isend']);//是否有结束时间
  $start_date = trim($_POST['start_date']);//开始日期
  $end_date = trim($_POST['end_date']);//结束日期
  $start_time = $_POST['start_time'];//开始时间
  $end_time = $_POST['end_time'];//结束时间
  if(ii_isnull($start_date)) $start_date = ii_format_date(ii_now(),'1');
  if(ii_isnull($end_date)) $end_date = ii_format_date(ii_now(),'1');
  if($isallday==1 && $isend==1){
      ...
      if(ii_isnull($start_time)) $start_time = ii_format_date(ii_now(),'23');
      $start_time .=':00+00:00';
      if(ii_isnull($end_time)) $end_time = ii_format_date(ii_now(),'23');
      $end_time .=':00+00:00';
...
function wdja_cms_admin_manage_editdisp()
{
  ...
  " . ii_cfname('start') . "='" . $start . "',
  " . ii_cfname('end') . "='" . $end . "',
  " . ii_cfname('isallday') . "='" . ii_get_num($_POST['isallday']) . "',
  " . ii_cfname('isend') . "='" . ii_get_num($_POST['isend']) . "',
...
function wdja_cms_admin_manage_edit()
{
  ...
  if ($trs)
  {
    ...
    foreach ($trs as $key => $val)
    {
      ...
      if($tkey == 'start'){
          $start_date = ii_get_lrstr($val, 'T', 'left');
          $start_time = ii_get_lrstr($val, 'T', 'rightr');
          $start_time = ii_get_lrstr($start_time, '+', 'left');
          $start_time = ii_get_lrstr($start_time, ':', 'leftr');
          $tmpstr = str_replace('{$start_date}', $start_date, $tmpstr);
          $tmpstr = str_replace('{$start_time}', $start_time, $tmpstr);
      }else if($tkey == 'end'){
          $end_date = ii_get_lrstr($val, 'T', 'left');
          $end_time = ii_get_lrstr($val, 'T', 'rightr');
          $end_time = ii_get_lrstr($end_time, '+', 'left');
          $end_time = ii_get_lrstr($end_time, ':', 'leftr');
          $tmpstr = str_replace('{$end_date}', $end_date, $tmpstr);
          $tmpstr = str_replace('{$end_time}', $end_time, $tmpstr);
      }
      else{
...
function wdja_cms_admin_manage_list()
{
  ...
  if (is_array($trsary))
  {
    foreach($trsary as $trs)
    {
      ...
      $tmptstr = str_replace('{$start}', ii_get_date($trs[ii_cfname('start')]), $tmptstr);
      $tmptstr = str_replace('{$end}', ii_get_date($trs[ii_cfname('end')]), $tmptstr);
