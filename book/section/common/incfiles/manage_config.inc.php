...
function wdja_cms_admin_manage_adddisp()
{
  ...
  $tbook_id = ii_get_safecode($_POST['book_id']);
...
function wdja_cms_admin_manage_list()
{
  ...
  $tbook_id = ii_get_num($_GET['book_id']);
  ...
  if (!ii_isnull($tbook_id) && $tbook_id != 0) $tsqlstr .= " and " . ii_cfname('book_id') . "=" . $tbook_id;
  ...
  if (is_array($trsary))
  {
    foreach($trsary as $trs)
    {
      ...
      $tmptstr = str_replace('{$book_id}', ii_get_num($trs[ii_cfname('book_id')],'0'), $tmptstr);
      $tmptstr = str_replace('{$book}', mm_get_field('book',ii_get_lrstr($trs[ii_cfname('book_id')], ',', 'left'),'topic'), $tmptstr);
      $tmptstr = str_replace('{$chapter_id}', ii_get_num($trs[ii_cfname('chapter_id')],'0'), $tmptstr);
      if(ii_get_num($trs[ii_cfname('chapter_id')],'0') != 0) $tmptstr = str_replace('{$chapter}', mm_get_field('book/chapter',ii_get_lrstr($trs[ii_cfname('chapter_id')], ',', 'left'),'topic'), $tmptstr);
  ...
  $tmpstr = str_replace('{$book_id}', $tbook_id, $tmpstr);
