...
function wdja_cms_module_list()
{
  ...
  if (ii_cache_html($tappstr))
  {
    ...
    for ($ti = 0; $ti < count($tgenres); $ti ++)
    {
      ...
      $tsid = $tidfield;
      $tsqlstr .= $tsid . " as un_sid," . ii_cfnames($tfpre, 'time') . " as un_time,'" . $tgenre . "' as un_genre from " . $tdatabase . " where " . ii_cfnames($tfpre, 'hidden') . "=0 and " . ii_cfnames($tfpre, 'lng') . "='$nlng'";
