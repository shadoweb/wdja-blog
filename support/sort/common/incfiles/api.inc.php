...
function mm_get_subsort_count($id)
{
  //获取子分类数量
  global $conn, $variable, $sort_database, $sort_idfield, $sort_fpre;
  $tsqlstr = 'select count(*) as recount from '. $sort_database.' where '. ii_cfnames($sort_fpre,'fsid').' = "' .$id.'" and '. ii_cfnames($sort_fpre,'hidden').' = 0';
  $trs = ii_conn_query($tsqlstr, $conn);
  $trs = ii_conn_fetch_array($trs);
  $total = $trs['recount'];
  return $total;
...
function mm_sel_sort_list($genre, $lng, $sid)
{
  ...
  if (is_array($tary))
  {
    ...
    foreach ($tary as $key => $val)
    {
      ...
      $tfgourl = mm_get_sort_field($val['fsid'],'gourl');
