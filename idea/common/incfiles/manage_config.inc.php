...
function wdja_cms_admin_manage_adddisp()
{
  ...
  $tcontent = ii_cstr($_POST['content']);
  if (!(ii_isnull($tcontent)))
  {
...
function wdja_cms_admin_manage_editdisp()
{
  ...
  if (!(ii_isnull($tcontent)))
  {
    ...
    " . ii_cfname('content') . "='" . ii_left($tcontent, 10000) . "',
...
function wdja_cms_admin_manage_list()
{
  ...
  if (is_array($trsary))
  {
    foreach($trsary as $trs)
    {
      ...
      $tmptstr = str_replace('{$content}', ii_htmlencode($trs[ii_cfname('content')], 1), $tmptstr);
