...
{//OK
  ...
  $npagenums = mysqli_num_rows($trs);
  $npages = @ceil($npagenums/$npagesize);
  for($i=0;$i<$npages;$i++){
    $turlkey = $i * $npagesize;
    $turls = $turl . '/'.ii_iurl('list', 0, $nurltype, 'urlkey=' . $turlkey);
...
{//OK
  ...
  $nidfield = $variable[ii_cvgenre($ngenre) . '.nidfield'];
  $nfpre = $variable[ii_cvgenre($ngenre) . '.nfpre'];
  $tsqlstr = "select $nidfield from $ndatabase where " . ii_cfnames($nfpre, 'hidden') . "=0";
  $trs = ii_conn_query($tsqlstr, $conn);
  if($trs){
      ...
      for($i=0;$i<$npages;$i++){
        $topic = ii_itake('global.support/global:basic.web_name','lng');
        if($i != 0) $topic .= '_'.$i;
...
function pp_get_home_list_html()
{//OK
  ii_conn_init();
  global $conn,$variable,$nurlpre,$nurltype,$slng;
  $array = array();
...
function wdja_cms_admin_manage_createdisp()
{
  ...
  $tbackurl = $_GET['backurl'];
    ...
    $arrays = array_merge($arrays,pp_get_home_list());
...
function wdja_cms_admin_manage_createdisp_html()
{
  ...
  if (file_exists($tburl))
  {
    $arrays = array_merge($arrays,pp_get_home_list_html());
