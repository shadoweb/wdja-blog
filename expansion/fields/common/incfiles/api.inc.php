...
function api_fields_convertUrlQuery($query)
{
  ...
  foreach ($queryParts as $param) {
    $item = explode('=', $param);
    $f = substr($item[0],0,1);
    $num = str_replace($f,'',$item[0]);
    if ($f == 'f' && is_numeric($num) && $item[1] != 0) $params[$item[0]] = $item[1];
  }
  return $params;
}
...
function api_list_form($classid,$offset,$type='0') {
  //前台筛选显示调用
  global $ngenre;
  if($type=='1') $tmpstr = ii_ireplace('global.expansion/fields:module.listformsort', 'tpl');
  else $tmpstr = ii_ireplace('global.expansion/fields:module.listform', 'tpl');
...
function api_list_sorts($cid = 0){
  global $ngenre,$nlng;
  if($cid == 0) return vv_isort($ngenre, 'tpl=global.expansion/fields:module.listsort;');
  $fid = mm_get_sort_field($cid,'fid');//多级分类ID组
  $fsid = mm_get_sort_field($cid,'fsid');//上级分类ID
  $subnum = mm_get_subsort_count($cid);//子分类数量
  $fid = ii_htmlencode(str_replace(',','_',$fid)).'_'.$cid;
  $tmpstr = api_list_sorts($fsid);
  if($subnum > 0){
    $tmpstr .= vv_isort($ngenre, 'tpl=global.expansion/fields:module.listsubsort;classid='.$cid);
    $tmpstr = str_replace('{$fsort}', mm_get_sorttext($ngenre,$nlng,$cid), $tmpstr);
    $tmpstr = str_replace('{$fsid}', $fsid, $tmpstr);
    $tmpstr = str_replace('{$fid}', $fid, $tmpstr);
    $tmpstr = str_replace('{$cid}', $cid, $tmpstr);
  }
  return $tmpstr;
}
