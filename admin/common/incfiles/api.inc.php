...
function api_get_view_url($genre,$gid,$source)
{
  global $variable,$conn,$nurlpre,$nurltype;
  $sid = api_get_related_sid($genre,$gid,$source);
  if($sid != 0) return $nurlpre.'/'.$source.'/'.ii_iurl('detail', $sid, $nurltype);
}

function api_get_view_topic($genre,$gid,$source)
{
  global $variable,$conn;
  $sid = api_get_related_sid($genre,$gid,$source);
  if(strpos($sid,',') !==false) $sid = ii_get_lrstr($sid, ',', 'left');
  $ndatabase = $variable[ii_cvgenre($source) . '.ndatabase'];
  $nidfield = $variable[ii_cvgenre($source) . '.nidfield'];
  $nfpre = $variable[ii_cvgenre($source) . '.nfpre'];
  if(!ii_isnull($ndatabase)){
      $tsqlstr = "select * from $ndatabase where $nidfield = $sid";
      $trs = ii_conn_query($tsqlstr, $conn);
      $trs = ii_conn_fetch_array($trs);
      if ($trs) return ii_htmlencode($trs[ii_cfnames($nfpre,'topic')]);
  }
...
function api_get_view_desc($genre,$gid,$source)
{
  global $variable,$conn;
  $sid = api_get_related_sid($genre,$gid,$source);
  if(strpos($sid,',') !==false) $sid = ii_get_lrstr($sid, ',', 'left');
  ...
  if(!ii_isnull($ndatabase)){
      ...
      $trs = ii_conn_query($tsqlstr, $conn);
      $trs = ii_conn_fetch_array($trs);
      if ($trs) return ii_htmlencode($trs[ii_cfnames($nfpre,'description')]);
  }
}

function api_get_view_image($genre,$gid,$source)
{
  global $variable,$conn;
  $sid = api_get_related_sid($genre,$gid,$source);
  if(strpos($sid,',') !==false) $sid = ii_get_lrstr($sid, ',', 'left');
  ...
  if(!ii_isnull($ndatabase)){
      ...
      $trs = ii_conn_query($tsqlstr, $conn);
      $trs = ii_conn_fetch_array($trs);
      if ($trs) return ii_htmlencode($trs[ii_cfnames($nfpre,'image')]);
  }
}

function api_get_view_sorttext($genre,$gid,$source)
{
  global $variable,$conn,$nlng;
  $sid = api_get_related_sid($genre,$gid,$source);
  if(strpos($sid,',') !==false) $sid = ii_get_lrstr($sid, ',', 'left');
  $ndatabase = $variable[ii_cvgenre($source) . '.ndatabase'];
  $nidfield = $variable[ii_cvgenre($source) . '.nidfield'];
  ...
  if(!ii_isnull($ndatabase)){
      ...
      $sorttext = '';
      if ($trs){
        $class = ii_get_num($trs[ii_cfnames($nfpre,'class')],'0');
        if($class != 0) $sorttext = mm_get_sorttext($source, $nlng, $class);
      }
      return $sorttext;
  }
}
